...
int main(){
  ...
  int n_termalizzazione = 3e4;

  ofstream Term;
  Term.open("../../Risultati/Gas/output_restart.dat",ios::app);
  ...
  for(int iblk=1; iblk <= nblk; ++iblk){ //Simulation
	   ...
	   Reset(iblk);   //Reset block averages
        ...
        if(istep%10 == 0){
          ...
          //Print_Istant(); SCOMMENTARE SE SI DESIDERANO LE GRANDEZZE ISTANTANEE SU FILE
...
void Input(int mode){ //Prepare all stuff for the simulation
  //mode ==1 se vuoi partire da una configurazione vecchia
 ...
 // double ep, ek, pr, et, vir;
  ...
  vtail = 0.;//(8.0*pi*rho)/(9.0*pow(rcut,9)) - (8.0*pi*rho)/(3.0*pow(rcut,3));
  ptail = 0.;//(32.0*pi*rho)/(9.0*pow(rcut,9)) - (16.0*pi*rho)/(3.0*pow(rcut,3));
...
void Restart(void) {  //processo di termalizzazione con una temperatura target
	// r(t-dt)= xold2
        // r(t) =xold[i]
	//r(t+dt) = x[i]
...
void Termalizzazione(){     //processo che termalizza il sistema a partire da una temperatura più alta o bassa a seconda della fase che si considera
  double sumv[3] = {0.0, 0.0, 0.0};
...
void Print_Istant(){  //stampa i valori istantanei di pressione, energia potenziale
    ofstream Epot, Pres, Ekin, Temp, Etot;
    Epot.open("../../Risultati/Gas/output_epot.dat",ios::app);
    Pres.open("../../Risultati/Gas/output_pres.dat",ios::app);
    Temp.open("../../Risultati/Gas/output_temp.dat",ios::app);
    Etot.open("../../Risultati/Gas/output_etot.dat",ios::app);
    Ekin.open("../../Risultati/Gas/output_ekin.dat",ios::app);
...
//istant Temperature
    Temp << walker[it] << endl;
//Istant kinetic Total_Energy
    Ekin << walker[ik] <<endl;
//Istant total Total_Energy
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    Epot.open("../../Risultati/Gas/ave_epot.out",ios::app);
    Pres.open("../../Risultati/Gas/ave_pres.out",ios::app);
    Gofr.open("output.gofr.0",ios::app);
    Gave.open("output.gave.0",ios::app);
    Temp.open("../../Risultati/Gas/ave_temp.out",ios::app);
    Ekin.open("../../Risultati/Gas/ave_ekin.out",ios::app);
    Etot.open("../../Risultati/Gas/ave_etot.out",ios::app);
