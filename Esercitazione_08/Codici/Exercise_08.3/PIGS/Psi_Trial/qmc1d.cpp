...
double variationalWaveFunction(double v)
{
	double _mu = 0.797;
	double _sigma = 0.614;
	double pezzo1 = exp(-pow(v-_mu,2)/(2.*pow(_sigma,2)));
	double pezzo2 = exp(-pow(v+_mu,2)/(2.*pow(_sigma,2)));
...
{
	double _mu = 0.797;
	double _sigma = pow(0.614,2);
	double e1 = pow(v-_mu, 2)/_sigma;
	double e2 = pow(v+_mu, 2)/_sigma;

	double psi_sec = ( -variationalWaveFunction(v) + e1 * exp(-e1/2.) + e2 * exp(-e2/2.) )/_sigma;
	return psi_sec;
...
{
	ofstream out("../../../../Risultati/probability_pigs_psiT.dat");
...
void readInput()
{
	...
	ifstream input_file("input.pigs");
